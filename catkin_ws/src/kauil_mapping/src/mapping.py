...
With a transform matrix T, H1316884you can transform a number of points at once by collecting the points in a numpy array
...
class Mapper:
    def __init__(self, map_width, map_height, map_resolution):
        ...
        self.map.data = np.full(map_width * map_height, fill_value=-1, dtype=np.int8)
        self.map.data[:] = -1  # Unknown
        self.map2d = np.full(
            (map_width, map_height), fill_value=-1, dtype=np.int8
        )  # For computation
        self.scan = None
        self.odom = None
    ...
    def mapit(self):
        while not rospy.is_shutdown():
            print("scan: {}, odom: {}".format(self.scan, self.odom))
            if self.scan is not None and self.odom is not None:
                ...
                orig_m, xy_m = scan_to_map_coordinates(
                    self.scan, self.odom, self.map.info.origin
                )
...
def ray_to_pixels(xr, yr, x, y, map_resolution, map):
    ...
    for i in range(int(n_pixels)):
        ...
        x_ = int(xp + round(i * v[0]))
        y_ = int(yp + round(i * v[1]))
        map[y_, x_] = 0
...
def polar_to_cartesian(r, th):
    ...
    if np.isnan(r):
        r = 6.0
    x = round(r * np.cos(th), 8) + 0.0  # To fix -0.0
    y = round(r * np.sin(th), 8) + 0.0  # To fix -0.0
    # -----------------------------------------------------
